...
SampleHold sampHold1,sampHold2;
...

{
    ...
    float slewKnobber1
        = fmap(hw.GetAdcValue(CV_8), .000001f, 0.11f, Mapping::LINEAR);
    float cvraw1 = hw.GetAdcValue(CV_6);

    float cvraw2 = hw.GetAdcValue(CV_7);
    float current_out1
        = sampHold1.Process(trig1, cvraw2, SampleHold::Mode::MODE_SAMPLE_HOLD);
    float current_out2
        = sampHold2.Process(trig2, cvraw1, SampleHold::Mode::MODE_SAMPLE_HOLD);
