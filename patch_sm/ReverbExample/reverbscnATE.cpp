...
// Add allpass filters before the main process function
AllpassDelay allpass_delays[2]; // Example for two allpass delays
...
int ReverbSc::Init(float sr)
{
    ...
    allpass_delays[0].Init(0.005f, 0.7f); // 5ms delay, 0.7 feedback
    allpass_delays[1].Init(0.010f, 0.7f); // 10ms delay, 0.7 feedback
                      ...
                      float *      out2)
{
    ...
    // Process input through allpass filters
    a_in_l = allpass_delays[0].Process(in1);
    a_in_r = allpass_delays[1].Process(in2);
