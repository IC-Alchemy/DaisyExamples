...
class Pooper
{
  public:
    ...
    void SetLoop(const float loop_start, const float loop_length)
    {
        ...
        _pending_loop_start
            = static_cast<size_t>(loop_start * (_buffer_length - 1));
        ...
        _pending_loop_length = std::max(
            kMinLoopLength, static_cast<size_t>(loop_length * _buffer_length));
    ...
    float Process(float in)
    {
        ...
        if(_rec_env_pos > 0)
        {
            ...
            float rec_attenuation = static_cast<float>(_rec_env_pos)
                                    / static_cast<float>(kFadeLength);
            _buffer[_rec_head] = in * rec_attenuation
                                 + _buffer[_rec_head] * (1.f - rec_attenuation);
            _rec_head++;
        ...
        {
            attenuation = static_cast<float>(_play_head)
                          / static_cast<float>(kFadeLength);
        }
        else if(_play_head >= _loop_length - kFadeLength)
        {
            attenuation = static_cast<float>(_loop_length - _play_head)
                          / static_cast<float>(kFadeLength);
        }
