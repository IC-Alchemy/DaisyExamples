...
void envelopePresets(int currentPreset, Adsr& env)
{
    ...
    env.SetTime(ADSR_SEG_DECAY, presetDecayTimes[currentPreset] + rel1 * .1f);
    env.SetTime(ADSR_SEG_RELEASE,
                presetReleaseTimes[currentPreset] + rel1 * .2f);
}
sampHoldStruct sampHolds[2];
...
void EnvelopeCallback(uint16_t** output, size_t size)
{
    ...
    env2.SetSustainLevel(.75f);

    rel1 = 1.f - (hw.GetAdcValue(ADC_11));


    env1.SetAttackTime(0.025f * randy2);
    env1.SetTime(ADSR_SEG_DECAY, (rel1 * .15) + randy1);
    env1.SetTime(ADSR_SEG_RELEASE, (rel1 * .9) + .08 + randy1);

    env2.SetAttackTime(0.0001f);
    env2.SetTime(ADSR_SEG_DECAY, rel1 * .2f + randy3);
    env2.SetTime(ADSR_SEG_RELEASE, rel1 * .3f + randy3 + 0.02f);
...

    {
        ...
        output[1][i] = env2.Process(env_state1) * 4095.0f;
    }
}
...
int main(void)
{
    ...
    Sync::Config cfg;
    cfg.generation_interval_mks = kMicroSecInSec / hw.AudioCallbackRate();
